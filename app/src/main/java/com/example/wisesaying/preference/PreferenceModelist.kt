...
object PreferenceModelist {

//새로운 명언을 추가하지 않은 기본 데이터들만 저장한 값
fun setModelListPref(context: Context, newModel: List<Pigme>) {
...
fun getModelListPref(context: Context): List<Pigme> {
    val prefs = PreferenceManager.getDefaultSharedPreferences(context)
...
fun setModelListPrefSelfStory(context: Context, newModel: List<Pigme>) {
    val prefs = PreferenceManager.getDefaultSharedPreferences(context)
    val editor = prefs.edit()
    val textStory = JSONArray()
    val image = JSONArray()
...
fun getModelListPrefSelfStory(context: Context):List<Pigme> {
    val prefs = PreferenceManager.getDefaultSharedPreferences(context)
